...
A candidate is accepted only when the restored repo verifies successfully. If multiple candidates pass, prefer the one produced by the more compressed artifact. Do not mark LLM-eligible source files for exact preservation.
...
- `repair_enabled`: use failed tests/lints/builds to patch restored files when useful.
...
1. Start with broad compression, repair enabled, and multiple candidates.
2. If syntax/import/lint failures name a file, switch to smaller chunks, a stricter format, stronger prompts, more candidates, or repair. Do not add source-file lossless overrides.
...
4. If decompressions vary, increase `candidate_count`; if they are stable but wrong, change prompt/format/model.
